namespace PVT.Shop.Web.Services
{
    ...
    public class CategoryService : ICategoryService
    {
        ...
        public Category GetCategory(int id)
        {
            ...
            category.Childs = this._categoryRepository.GetAll(c => c.Parent.Id == id).OrderBy(c => c.Name).ToList();
        ...
        public CategoryProductsViewModel SearchCategoryProducts(string q, int categoryId = 1, int page = 1, int pageSize = 30)
        {
            var categoriesId = this.GetChildsId(categoryId);
            var products = this._productRepository.GetAll(p => categoriesId.Any(id => id == p.CurrentCategoryId)).Where(p => p.Name.ToLower().Contains(q.ToLower()) && p.Display.Equals(true)).ToList();
        ...
        private void CreateCategory(Category newCategory)
        {
            ...
            addedCategory.Parent = this._categoryRepository.GetById(newCategory.Parent.Id) ??
                                   this._categoryRepository.GetById(1);
            this._categoryRepository.Save();
        ...
        private List<int> GetChildsId(int categoryId)
        {
            ...
            var ids = this._categoryRepository.GetAll(c => c.Parent.Id == categoryId).Select(c => c.Id).ToList();
